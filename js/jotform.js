                    ...
                    try {
                        ...
                        $$('.form-pagebreak-next').each(function (nextButton) {
                            ...
                            if (JotForm.isSourceTeam) {
                                function createErrorNavigationButton(type, text, clickHandler) {
                                    var element = new Element('button', { type: 'button', className: 'form-error-navigation-button form-error-navigation-button-' + type });
                                    element.insert(text);
                                ...
                                function isActiveElementErrored() {
                                    return $(document.activeElement).hasClassName('form-validation-error');
                                }
                                function getErroredFields() {
                                    return $$('.form-line-error:visible');
                                }
                                function getNumberOfErroredFields() {
                                    var erroredFields = getErroredFields();
                                    return erroredFields.length;
                                }
                                function getIndexOfActiveErroredField() {
                                    var erroredFields = getErroredFields();
                                    var activeLine = $(document.activeElement).up('.form-line');
                                    return erroredFields.indexOf(activeLine);
                                }
                                function focusErroredField(index) {
                                    var erroredFields = getErroredFields();
                                    var nextErroredFieldLine = erroredFields[index];
                                    if (!nextErroredFieldLine) {
                                        return;
                                    }
                                    
                                    nextErroredFieldLine.scrollIntoView();
                                    var firstInput = nextErroredFieldLine.down('input[type!="tel"],select,textarea');
                                    if (firstInput) {
                                        firstInput.focus();
                                    }
                                }
                                function getIndexOfNextErroredField(direction) {
                                    if(!isActiveElementErrored) {
                                        return 0;
                                    }

                                    var directionConstant = direction === 'next' ? 1 : -1;
                                    var numberOfErroredFields = getNumberOfErroredFields();
                                    var activeErroredFieldIndex = getIndexOfActiveErroredField();
                                    return (numberOfErroredFields + (activeErroredFieldIndex + directionConstant)) % numberOfErroredFields;
                                }
                                function getIndexOfPrevErroredField() {
                                    if(!isActiveElementErrored) {
                                        return getErroredFields().length;
                                    }
                                }
                                function focusNextErroredField() {
                                    var nextErroredFieldIndex = getIndexOfNextErroredField('next');
                                    focusErroredField(nextErroredFieldIndex);
                                }
                                function focusPrevErroredField(e) {
                                    var prevErroredFieldIndex = getIndexOfNextErroredField('back');
                                    focusErroredField(prevErroredFieldIndex);
                                }
                                function generateErrorMessage() {
                                    var errorMessageContainer = new Element('div', { className: 'form-error-navigation-message' });

                                    var numberOfErroredFields = getNumberOfErroredFields();
                                    var numberMessageContainer = new Element('div', { className: 'form-error-navigation-error-count' });
                                    numberMessageContainer.insert(numberOfErroredFields + ' error');

                                    errorMessageContainer.insert(numberMessageContainer).insert('Please correct them before continue');
                                    return errorMessageContainer;
                                }
                                var navigationBox = new Element('div', { className: 'form-error-navigation-container' });
                                var prevNavButton = createErrorNavigationButton('prev', '<', focusPrevErroredField);
                                var nextNavButton = createErrorNavigationButton('next', '>', focusNextErroredField);
                                var errorMessageBox = generateErrorMessage();
                                navigationBox.insert(errorMessageBox).insert(prevNavButton).insert(nextNavButton);
                                $(nextButton.parentNode.parentNode).insert(navigationBox);
                                focusErroredField(0);
                                return;
                            }
