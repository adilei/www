...
class FM_SubmissionSerializer
{
    ...
    public function __construct(&$config,&$logger,&$error_handler,$tablename)
    {
       $this->tablename = $tablename;
        ...
        $this->dbutil->AddField('Serialized','TEXT');        
    }
    
    function SerializeToTable($var_array)
    {
        ...
        if(!$this->dbutil->Ensuretable($this->tablename))
        {
            return false;
        }
        
        $ret = $this->InsertIntoTable($var_array);
    ...
    function RecreateFromTable($id,&$var_array,$reset=true)
    {
        $this->logger->LogInfo("SubmissionSerializer RecreateFromTable: ID $id");
        
        if(!$this->dbutil->Login())
        {
            $this->logger->LogError("RecreateFromTable Login error");
            return false;
        }
        
        $serialized = $this->GetSerializedData($id);
        if(false === $serialized)
        {
            $this->logger->LogError("RecreateFromTable Error getting previous post data");
            return false;
        }
     
        $var_array = unserialize($serialized);
    ...
    function Login()
    {
        if(!$this->dbutil->Login())
        {
            $this->logger->LogError("FM_SubmissionSerializer::Reset Login error");
            return false;
        }
        return true;
    }
    ...
    function GetSerializedData($id)
    {
        $qry = "SELECT Serialized FROM $this->tablename WHERE ID=$id";
        
        $result = mysqli_query($this->dbutil->GetConnection(), $qry );
        
        if(!$result || mysqli_num_rows($result) <= 0)
        {
            /*Note: Paypal may send duplicate IPNs occassionaly. So only log the error. do not send error email to user*/
            $this->logger->LogError("Error retrieving saved form submission No results for ID $id");
            return false;
        }
    ...
    function InsertIntoTable($var_array)
    {
        $serialized = serialize($var_array);
        $fieldval = $this->dbutil->EscapeValue($serialized,'text');
        $qry ="INSERT INTO $this->tablename (Serialized) VALUES ($fieldval);";
        
        if(!mysqli_query($this->dbutil->GetConnection(), $qry))
        {
            $this->dbutil->HandleError('Error inserting data to the table '.$this->tablename,"\nquery:$qry");
            return false;
        } 
        return mysqli_insert_id($this->dbutil->GetConnection());
    }
    ...
    function DeleteRow($id)
    {
        ...
        if(!mysqli_query($this->dbutil->GetConnection(), $qry))
        {
            $this->logger->LogError("FM_SubmissionSerializer error deleting row from the table. ID: $id ".$this->dbutil->GetError());
            return false;
        }
        return true;
    }
