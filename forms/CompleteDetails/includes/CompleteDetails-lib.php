...
class FM_FileUploadHandler extends FM_Module
{
    private $loaded_files;
    private $file_upload_session_var;
	private $loaded_from_session;
    ...
    public function __construct()
    {
        ...
        $this->file_upload_session_var='sfm_fileupload_list';
		$this->loaded_from_session = false;
		$this->field_validations = array();
    }
    function SetFileUploadFields($fields)
    {
        foreach($fields as $f)
        {
            array_push($this->upload_fields,$f);
        }        
    }
	...
	function setFieldValidations($fieldname,$command,$command_value,$error)
	{
		if(!isset($this->field_validations[$fieldname]))
		{
			$this->field_validations[$fieldname] = array();
		}
		array_push($this->field_validations[$fieldname],
				array('command'=>$command,'command_value'=>$command_value,'error'=>$error));
	}
	
	function OnInit()
	{
		$this->LoadFileListFromSession();
	}
	
	function GetUniqueFileName($destfile)
	{
        ...
        {   
            ...
            $barename = basename($path_parts["basename"],".".$path_parts["extension"]);
            do
            {
                $destfile = $path_parts["dirname"]."/".
                    $barename."$i.".$path_parts["extension"];
                $i++;
            }
    ...
    function CopyFile_Prevent_Overwrite($sourcefile,$destfile)
    {
		$destfile = $this->GetUniqueFileName($destfile);
        if(!copy($sourcefile,$destfile))
        {
            $this->logger->LogError("CopyFile_Prevent_Overwrite file copy failed. ".
                            "source: $sourcefile dest:$destfile");
        }
        if(!@chmod($destfile,0664))
		{
			$this->logger->LogError("chmod on $destfile failed");
		}
        return $destfile;
    }
    
    function get_tmpuploadfolder()
    {
        return sfm_make_path($this->getFormDataFolder(),"t_uploads_".$this->formname);
    }

    function get_uploadfiles_folder()
    {
        return $this->config->get_uploadfiles_folder();
    }
    ...
    function DoAppCommand($cmd,$val,&$app_command_obj)
    {
        ...
        switch($cmd)
        {
            ...
            {
                $this->get_attachment_list($app_command_obj->response_sender);
                $ret=true;
                break;
            }
            case 'get_attachment':
            {
                $this->get_attachment($val,$app_command_obj->response_sender);
                $ret=true;
                break;
            }
            case 'rem_file':
    ...
    {
        if(!empty($_GET['sfm_postfile']))
        {
			
			$this->handle_ajax_file_upload();
            return false;
        }
		elseif(!empty($_GET['sfm_filepreview']))
		{
			
			$this->show_file_preview($_GET['sfm_filepreview']);
			return false;
		}
		elseif(!empty($_GET['sfm_upload_file_delete']))
		{
			$id = trim($_GET['sfm_upload_file_delete']);
			if(true === $this->delete_uploaded_file($id))
			{
				echo "success";
			}
			else
		...
		elseif(!empty($_GET['sfm_upload_getlist']))
		{
			$fieldname = trim($_GET['sfm_upload_getlist']);
			if(empty($fieldname)){ return false; }
			
			$this->get_file_list_json($fieldname);
			return false;
		}
		return true;
	}
    ...
    function Process(&$continue)
    {
        $this->HandleNativeFileUpload();
		$this->update_formvars();
        $continue=true;
    }
	
	function show_file_preview($id)
	{
		$file_obj = $this->get_file_from_id($id);
		$filepath = $file_obj['loaded_file'];
		
		if(true ===  $this->ext_module->HandleFilePreview($filepath))
		{
			
			//do nothig; preview is shown.
		}
		else
		{//no preview
			header("HTTP/1.0 404 Not Found - No preview");
		}
	}
	...
	function delete_uploaded_file($id)
	{
		...
		$nFiles = count($this->loaded_files);
		for($f=0;$f<$nFiles;$f++)
        {
			if($this->loaded_files[$f]['id'] == $id)
			{
				$filepath = $this->loaded_files[$f]['loaded_file'];
				unlink($filepath);
				unset($this->loaded_files[$f]);
				$this->SaveFileListToSession();
				return true;
			}//if
		}//for
		return false;
	...
	function validateAjaxUpload($fieldname,&$error)
	{
		if(empty($this->field_validations[$fieldname]))
		{ 
			return true; 
		}
		
		foreach($this->field_validations[$fieldname] as $valobj)
		{
			$default_error_message='';
			
			$ret = $this->ValidateFileUpload($fieldname,$valobj['command'],
					$valobj['command_value'],$default_error_message,/*form_submitted*/false);
			if(false == $ret)
			{ 
				$error = empty($valobj['error'])?$default_error_message:$valobj['error'];
				return false; 
			}
		}
	...
	function ValidateFileUpload($variable_name,$command,$command_value,&$default_error_message,$form_submitted=true)
    {
        $bret=true;

        if($command != 'max_numfiles' && $this->IsAlreadyLoadedFile($variable_name))
        {
            return true;
        }
		...
		$direct_upload = false;
		$file_obj=array();
		if(!empty($this->globaldata->files) || isset($this->globaldata->files[$variable_name]))
		{
			$direct_upload = true;
			$file_obj = $this->globaldata->files[$variable_name];
		}
        ...
        switch($command)
         {
			case "req_file":
			 { 
				...
				{//do not validate in case of ajax upload
				   if(empty($file_obj) ||
				      empty($file_obj['size']) || 
					  $file_obj['size'] <= 0) 
					{
						$bret=false;
						$default_error_message = 
							sprintf("A file need to be uploaded for %s",$variable_name);
					}//if
				}//if
				break;
			 }//case
         ...
         { 
			...
			if(!empty($file_obj))
			{
				if($file_obj['size'] > $maxsize ||
					@filesize($file_obj['tmp_name']) > $maxsize) 
				{
					$bret=false;
					$default_error_message = 
						sprintf("%s: The file has exceeded the size limit %d",$variable_name,$maxsize);
				}//if
			}
            break;
        }//case max_filesize
         ...
         case "file_extn":
            {
               if(!empty($file_obj) && !empty($file_obj['name']))
               {
                    $parts = pathinfo($file_obj['name']);
					if(empty($parts["extension"])){	break; }
                    ...
                    if(!$found)
                    {
                        $bret=false;
                        $default_error_message = 
                            sprintf("%s: Only the following types of file extensions are allowed: %s",
		 ...
		 case "max_numfiles":
            {
				$num_max_files = intval($command_value);
				
				if($this->get_numfiles_uploaded($variable_name) >= $num_max_files)
				{
					$bret=false;
                    $default_error_message = 
                            sprintf("%s: Exceeded the maximum number of uploads (%d) ",$variable_name,$num_max_files);
				}
				break;
			}
        }//switch
	...
	function update_formvars()
	{
		foreach($this->upload_fields as $filefield)
        {
			$this->formvars[$filefield] = $this->GetUploadedFileName($filefield);
            $this->logger->LogInfo("update_formvars $filefield:".$this->formvars[$filefield]);
		}
	}
	
	function get_file_list_json($fieldname)
	{
		$list = array();
		$nFiles = count($this->loaded_files);
		for($f=0; $f<$nFiles; $f++)
		{
			if(($this->loaded_files[$f]['input_name'] ==  $fieldname) && 
				!empty($this->loaded_files[$f]['id']))
			{
				$file = new stdClass();
				$file->name = $this->loaded_files[$f]['orig_file_name'];
				$file->size = $this->loaded_files[$f]['size'];
				$file->id = $this->loaded_files[$f]['id'];
				$list[] = $file;
			}
		}	
	...
	function AttachFilesToDownload()
	{
		$response = new FM_Response($this->config,$this->logger);
		
		$this->get_attachment_list($response);
		...
		foreach($arr_files as $filename)
		{
			...
			$this->get_attachment("$filename",$file_response);			
			echo sfm_targz('uploads/'.$filename,$file_response->getResponseStr());
		}		
	}
	
	function get_numfiles_uploaded($fieldname)
	{
		$nUploaded = 0;
		$nFiles = count($this->loaded_files);
    ...
    function handle_ajax_file_upload()
	{
		...
		foreach($this->upload_fields as $filefield)
        {
			if(!isset($_FILES[$filefield])){continue;}
			$error_msg='';
			if(!$this->validateAjaxUpload($filefield,$error_msg))
			{
				$err = new stdClass();
				$err->error=$error_msg;
				$info[] = $err;
			...
			elseif ($upload || isset($_SERVER['HTTP_X_FILE_NAME'])) 
			{
				$info[] = $this->handle_file_upload(
						...
						$this->get_upload_field('HTTP_X_FILE_NAME','name',$upload),
						$this->get_upload_field('HTTP_X_FILE_SIZE','size',$upload),
						$this->get_upload_field('HTTP_X_FILE_TYPE','type',$upload),
						$filefield );
			}
		}
	...
	function get_upload_field($xfield,$field,$upload)
	{
		$ret = null;
		if(isset($_SERVER[$xfield]))
		{
			$ret = $_SERVER[$xfield];
		}
		elseif(isset($upload[$field]))
		{
			$ret = $upload[$field];
		}
		return $ret;
	...
	function handle_file_upload($uploaded_file, $name, $size, $type,$filefield) 
	{
		...
		if (empty($file->name)) 
		{ 
			$this->logger->LogError("handle_file_upload: File name is empty!");
			return; 
		}
		
		$upload_dir = $this->EnsureTmpUploadFolder();
		
		if(false === $upload_dir)
		{ 
			$this->logger->LogError("handle_file_upload: Failed creating upload folder!");
			return;
		}
		
		$file_path = $this->GetUniqueFileName($upload_dir.'/'.$file->name);

		clearstatcache();
		if ($uploaded_file && is_uploaded_file($uploaded_file)) 
		{
			//move_uploaded_file($uploaded_file, $file_path);
			$this->CopyFile_Prevent_Overwrite($uploaded_file, $file_path);
		} 
		else 
		{
		...
		{
			$err = new stdClass();
			$err->error = 'Error uploading the file. Please try uploading the file again. ';
			return $err;
		}
		
		$file->size = $file_size;
		
		$file->id = md5($file_path.$this->config->get_form_id());
	...
	/*
	Todo: move MkDir/Ensure folder to common_objs->utils class
	...
	*/
	function MkDir($folder)
	{
		...
		{
			$this->logger->LogError("chmod $folder failed.");
		}
		return true;
	}
	
	function EnsureTmpUploadFolder()
	{
        $upld_folder = $this->get_tmpuploadfolder();


		if(!file_exists($upld_folder) && !$this->MkDir($upld_folder))
		{
			$this->error_handler->HandleConfigError("File Upload handler: failed creating folder: $upld_folder");
			return false;
		}
		return $upld_folder;
	}
    ...
    //NOTE: LoadUploadedFiles is for native file uploads where the file is submitted along with the form submission
    ...
    function rem_file($val,&$app_command_obj)
    {
        ...
        if($is_attachment == 'a')
        {
            $base_folder = $this->get_uploadfiles_folder();
    ...
    function IsNonSecureFile($filename)
    {
        $nonsecfiles = array('html','htm','php','php2','php3','php4','php5',
                'phtml','pwml','inc','asp','aspx','ascx','jsp','cfm','cfc','pl',
                'bat','exe','com','dll','vbs','js','reg','cgi','htaccess','asis');
    ...
    function IsFieldFileUpload($fieldname)
    {
        foreach($this->loaded_files as $fileobj)
        {
            if($fileobj['input_name'] == $fieldname)
            {
                return true;
            }
        }
    ...
    function IsAlreadyLoadedFile($inputname)
    {
	  $this->LoadFileListFromSession();
	  $ret = $this->IsFieldFileUpload($inputname);
	  
      return $ret;
    }

    function GetUploadedFileName($fieldname)
    {
		$ret_filenames='';
        foreach($this->loaded_files as $fileobj)
        {
            ...
            {
				$filename='';
				if(empty($fileobj['saved_file']))
				{ 
					$filename = basename($fileobj['loaded_file']); 
				}
				else
				{
					$filename = basename($fileobj['saved_file']);
				}
				
				$ret_filenames .= $filename.',';
            }
        }
		return rtrim($ret_filenames,',');
    }
    ...
    {
        $upload_folder = $this->get_uploadfiles_folder();

        if(!file_exists($upload_folder) && !$this->MkDir($upload_folder))
        {
            $this->logger->LogError("File Upload handler: ".
                    "failed creating folder: $upload_folder");
            return false;
        }
        ...
        for($f=0;$f<$nFiles;$f++)
        {
            $save_path = $upload_folder."/".
                            basename($this->loaded_files[$f]['loaded_file']);

            if(empty($this->loaded_files[$f]['saved_file']))
            {
                $this->logger->LogInfo("File Upload handler: saving uploaded file: $save_path");

                $this->loaded_files[$f]['saved_file'] 
                    = $this->CopyFile_Prevent_Overwrite($this->loaded_files[$f]['loaded_file'],
                                                $save_path);
                $this->logger->LogError("File Upload handler:  saved path".$this->loaded_files[$f]['saved_file']);
            }
            else
            {
                $this->logger->LogInfo("File Upload handler: already loaded file $save_path");
            }
        }
        $this->update_formvars();
        return true;
    ...
    function CreateUploadFilesVar($session_var_name)
    {
        $this->SaveFileListToSession();
    }
	...
	{
		return $this->formname.'_'.$this->file_upload_session_var;
	}
    function HandleNativeFileUpload()
    {
        $this->LoadFileListFromSession();
        $this->LoadUploadedFiles();
        $this->SaveFileListToSession();
    }
    function SaveFileListToSession($session_var_name='')
    {
         $this->logger->LogInfo("SaveFileListToSession ".@print_r($this->loaded_files,true));
         $this->globaldata->session[$this->getSessionVarName()] = $this->loaded_files;
    }
    
    function LoadFileListFromSession($session_var_name='')
    {
	  if($this->loaded_from_session){ return; }
	  
      if(isset($this->globaldata->session[$this->getSessionVarName()]))
      {
         $this->loaded_files = 
            array_merge($this->loaded_files,$this->globaldata->session[$this->getSessionVarName()]);
         
         $this->logger->LogInfo("LoadFileListFromSession  loaded_files ".var_export($this->loaded_files,TRUE));
		 $this->loaded_from_session = true;
      }
    }
    
    function DeleteLoadedFiles()
    {
        foreach($this->loaded_files as $fileobj)
        {
            unlink($fileobj['loaded_file']);
        }        
    }

    function AttachFiles(&$mailer)
    {
        foreach($this->loaded_files as $fileobj)
        {
            $mailer->AttachFile($fileobj['loaded_file'],$fileobj['mime_type']);
        }//foreach
    }
    function Destroy()
    {
		...
		{
			$this->DeleteLoadedFiles();
		}
    }

    function get_attachment_list(&$response_sender)
    {
        $response='';
        ...
        if (!$handle)
        {
            $err = sprintf(_("Failed getting file list from directory: %s"),$upload_folder);
    ...
    function get_attachment($filename,&$response)
    {
        $upload_folder = $this->get_uploadfiles_folder();

        $att_path = sfm_make_path($upload_folder,$filename);
        
        return $response->load_file_contents($att_path);
    }
    
    function find_file_uploaded($fieldname)
    {
        $nFiles = count($this->loaded_files);
        for($f=0; $f < $nFiles; $f++)
        {
...
class FM_ThumbnailModule extends FM_ExtensionModule
{
    var $thumbnail_folder;
    var $thumbnail_width;
    var $thumbnail_width_bigger;
    
    public function __construct()
    {
        parent::__construct();
        $this->thumbnail_folder='';
        $this->thumbnail_width=60;
        $this->thumbnail_width_bigger=300;
        
        $this->cachingtime = 0;//60 * 60;//1 hr
    }
    
    function BeforeStartProcessing()
    {
		$this->thumbnail_folder = sfm_make_path($this->config->getFormDataFolder(),'thumbnails');
        return true;
    }
    ...
    function AfterVariablessInitialized()
    {
        ...
        if(!empty($_GET['sfm_tnail']))
        {
            $this->HandleCommand($_GET['sfm_tnail']);
            return false;//handled
        }
        return true;
    }
    
    function BeforeSubmissionTableDisplay(&$datarow)
    {
        
        foreach($datarow as $rptfield => $rptvalue)
        {
            if($this->IsDataTableFileField($rptfield))
            {
                $datarow[$rptfield] = $this->GetImageCode($rptvalue);
                
            }
        }
    }

    function BeforeDetailedPageDisplay(&$datarow)
    {
        foreach($datarow as $rptfield => $rptvalue)
        {
            if($this->IsDataTableFileField($rptfield))
            {
                $datarow[$rptfield] = $this->GetImageCode($rptvalue,/*bigger image*/true);
            }
        }    
    }
    
    function OnDeletingAttachment($filepath)
    {
        $filename = basename($filepath);
        $tnail_path = sfm_make_path($this->thumbnail_folder,$filename);
	...
	function HandleFilePreview($filepath)
	{
		...
		{
			$this->show_thumbnail($filepath,/*bigger*/false);
			return true;
		}
		return false;
	}
	
    function GetImageCode($filenames,$bigger_image=false)
    {
		$ret='';
		$arr_filenames = explode(',',$filenames);
		foreach($arr_filenames as $filename)
		{
			$ret .= $this->GetSingleImageCode($filename,$bigger_image);
			$ret .="\n";
		}
		$ret = rtrim($ret);
		return nl2br($ret);
	...
	function GetSingleImageCode($filename,$bigger_image)
	{
        $source_path = sfm_make_path($this->config->get_uploadfiles_folder(),$filename);
        if(!file_exists($source_path))
        {
            $this->logger->LogError(" File does not exist: $source_path");
            return $filename;
        }
        if($this->IsImageFile($filename))
        {
            return $this->GetImageLink($filename,$bigger_image);
        }
        else
        {   
    ...
    function GetImageLink($filename,$bigger_image=false)
    {
        $source_path = sfm_make_path($this->config->get_uploadfiles_folder(),$filename);
        ...
        $tnail_width = $bigger_image? $this->thumbnail_width_bigger : $this->thumbnail_width;
        
        $tnail_height = $this->GetTNailHeight($img_size[0],$img_size[1],$tnail_width);
        
        $img_size_str = " width='{$tnail_width}' height='{$tnail_height}' ";
        
        $imgurl='?sfm_tnail=img_get_tnail&filename='.urlencode($filename);
        if($bigger_image)
        {
            $imgurl .='&tnail_b=y';
        }
        $origimage_url = '?sfm_tnail=img_get_orig&filename='.urlencode($filename);
        
        return "<a class='sfm_tnail_link' href='$origimage_url'><img class='sfm_tnail_img' src='$imgurl' $img_size_str /></a>";    
    }
    
    function GetFileLink($filename)
    {
        $link_url = '?sfm_tnail=attach_get_file&filename='.urlencode($filename);
        $filename_disp =  htmlentities($filename);
        return "<a href='$link_url'>$filename_disp</a>";
    }

    function IsFileField($fieldname)
    {
        return $this->IsDataTableFileField($fieldname);      
    }
    function IsDataTableFileField($fieldname)
    {
        $type = $this->config->element_info->GetType($fieldname);
        
        if('file' == $type || 'upload_ex' == $type )
        {
            return true;
        }
        return false;
    ...
    function IsImageFile($filepath)
    {
        ...
        $ext = sfm_getfile_extension($filepath);
    ...
    function GetTNailHeight($orig_width,$orig_height,$twidth)
    {
        return floor( $orig_height * ( $twidth / $orig_width ) );
    }
	...
	*/
    function MkDir($folder)
	{
		...
		if(!@chmod($folder,0777))
		{
			$this->logger->LogError("Thumbnail maker chmod $folder failed.");
		}
		return true;
	}
    ...
    {
        if(strpos($this->thumbnail_folder,'thumbnails') === FALSE)
        {
            return;//nervous !
        }
        
        $files = glob(sfm_make_path($this->thumbnail_folder,"*"));
        foreach($files as $file) 
        {
            if(is_file($file) && $this->IsImageFile($file)) 
            { 
                if(time() - filemtime($file) >= 60*60)//1 hr //2*24*60*60)// 2 days
                {
                    unlink($file);
                }
            }
    ...
    function CreateThumbnail($sourcefile,$destfile,$twidth)
    {
        $this->clear_cache();
        if(!file_exists($this->thumbnail_folder) && !$this->MkDir(($this->thumbnail_folder)))
        {
            $this->logger->LogError("Failed creating thumbnail folder");
            return;
        }
        
        $ext = sfm_getfile_extension($sourcefile);
        ...
        $new_height = $this->GetTNailHeight($width,$height,$twidth);
        ...
        imagejpeg( $tmp_img, $destfile);
    }
    
    function ShowImageOrig($image_filename)
    {
        ...
        $this->ReturnImage($source_path);
    }
    
    function ReturnGeneralFile($filename)
    {
        ...
        $mime = sfm_get_mime_type($filename);
    ...
    function ReturnImage($image_path)
    {
        ...
        {//todo: log the error
            echo "error getting thumbnail";
        }           
    }
    
    function ShowImageThumbnail($image_filename,$bigger=false)
    {
        $filename = basename($image_filename);
        
        $source_path = sfm_make_path($this->config->get_uploadfiles_folder(),$filename);
		
		$this->show_thumbnail($source_path, $bigger);
    }
	
	function show_thumbnail($source_path,$bigger)
	{
		$filename = basename($source_path);
        if(!file_exists($source_path))
        {
			$this->logger->LogError("File $source_path does not exist");
            return;
        }
        if(true === $bigger)
        {
            $filename = 'sfmb_'.$filename;
        }
        else 
        {//This avoids caching for small images
            $filename = 'sm'.rand(1,1000).'_'.$filename;
        }
        $this->logger->LogInfo("Thumbnail file $filename");
        $tnail_path = sfm_make_path($this->thumbnail_folder,$filename);
		
        if(!$this->IsCacheValid($tnail_path))
        {
            $tnail_width = $bigger ? $this->thumbnail_width_bigger : $this->thumbnail_width;
            $this->CreateThumbnail($source_path,$tnail_path,$tnail_width);     
        }
        
        if(!file_exists($tnail_path))
        {
			$this->logger->LogError("show_thumbnail file $tnail_path does not exist");
            return;
        }
        
        $this->ReturnImage($tnail_path);	
	}
    
    function IsCacheValid($tnail_path)
    {
        if(!file_exists($tnail_path))
        {
            return false;
        }
        $filemtime = @filemtime($tnail_path);
        if (!$filemtime or (time() - $filemtime >= $this->cachingtime))
        {
            return false;
        }
        return true;
    ...
    {
        if($cmd == 'img_get_tnail' && !empty($_GET['filename']))
        {
            $bigger=isset($_GET['tnail_b'])?true:false;
            $this->ShowImageThumbnail($_GET['filename'],$bigger);
        }
        elseif($cmd == 'img_get_orig' && !empty($_GET['filename']))
        {
            $this->ShowImageOrig($_GET['filename']);
        }
        elseif($cmd == 'attach_get_file' && !empty($_GET['filename']))
        {
            $this->ReturnGeneralFile( $_GET['filename'] );
        }
    }
}

////////ConfirmPage/////////////////
class FM_DataHandler extends FM_Module
{
    public function __construct()
    {
        parent::__construct();
    ...
    {
        $continue = true;
        if(false === $this->ext_module->PreprocessFormSubmission($this->formvars))
        {
            $continue = false;
        }
        return true;
...
{
    private $formdata_file;
    private $csv_file_maxsize;//in KBs 
    private $csv_file_variables;
    private $csv_seperator;
    private $csv_filename;
    private $file_uploader;

	public function __construct($maxsize=1024,$filepath="")
	{
        parent::__construct();
...
class FM_AdminPageHandler extends FM_Module
{
    ...
    function AfterVariablessInitialized()
    {
        if(!empty($_GET['sfm_adminpage']))
        {
            ...
            {
                if(!empty($_GET['sfm_logout']) && 'yes' ==  $_GET['sfm_logout'])
                {
                    $this->LogOut();
                    return false;//handled
                }
